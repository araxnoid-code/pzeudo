...
fn embedding_test_1() {
    ...
    let len = train.len();
    let train_tensor =
        Tensor::param_from_slice_with_shape(&train, &[len], &module_builder, ReqNoGrad).unwrap();

    let test: Vec<f32> = train.iter().map(|val| *val + 1.0).collect();
    let test_tensor =
        Tensor::param_from_vector_with_shape(test, &[len, 1], &module_builder, ReqNoGrad).unwrap();
