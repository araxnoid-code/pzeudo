...
impl<F> GradStorage<F> {
    pub(crate) fn new(capacity: Option<usize>) -> GradStorage<F> {
        Self {
            update: Vec::with_capacity(capacity.unwrap_or(0)),
    ...
    pub fn set_update(&mut self, idx: usize, status: bool) -> Result<(), PzeudoErr> {
        *self.update
            .get_mut(idx)
            .ok_or(PzeudoErr::StorageErr(format!("GradStorage::set_update. index {idx} points to an invalid location on gradient storage(status).")))? = status;
        Ok(())
    }
