...
impl<F, T, G> Tensor<F, T, G> {
        ...
        F: Copy + Mul<Output = F>,
    {
        ...
        let mut record = self.get_record().borrow_mut();
        let record_label = RecordLabel::MulScalar(rhs_scalar, self.get_grad_idx(), grad_idx);
        ...
        F: Copy + Mul<Output = F>,
    {
        ...
        let mut record = self.get_record().borrow_mut();
        let record_label = RecordLabel::MulScalar(lhs_scalar, self.get_grad_idx(), grad_idx);
        let record_idx = RecordStatus::Record(record.len());
        record.push(record_label);
    ...
    F: Copy + AddAssign + Mul<Output = F>,
{
    if let Some(grad_idx) = grad_idx {
        ...
        if let Some(arr_grad_idx) = arr_grad_idx {
            ...
            for i in 0..len {
                *arr_grad.linear_index_mut(i)? += scalar * grad.linear_index(i)?;
            }
