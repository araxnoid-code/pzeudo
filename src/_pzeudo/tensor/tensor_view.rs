...
pub struct TensorView<'array, 'backward_label, F> {
    pub(crate) array: ArrayViewD<'array, F>,
...
impl<'array, 'bacward_label, F> TensorView<'array, 'bacward_label, F>
where
    F: Clone + Zero + One + Float + Add<Output = F> + Copy,
{
    pub fn new(
        array: ArrayViewD<'array, F>,
        gradient: Option<ArrayD<F>>,
        backward_label: Option<BackwardLabel<'bacward_label, F>>,
    ) -> TensorView<'array, 'bacward_label, F> {
        Self {
            array,
    ...
    pub fn from_array(array: ArrayViewD<'array, F>) -> TensorView<'array, 'bacward_label, F> {
        Self {
...
impl<'array, 'backward_label, F> TensorTrait<'backward_label, F>
    for TensorView<'array, 'backward_label, F>
where
    F: Clone + One + Zero,
{
...
impl<'array, 'bacward_label, F> PzeudoOpsAdd<'bacward_label, F>
    for TensorView<'array, 'bacward_label, F>
where
...
impl<'array, 'bacward_label, F> Display for TensorView<'array, 'bacward_label, F>
where
    F: Display,
{
