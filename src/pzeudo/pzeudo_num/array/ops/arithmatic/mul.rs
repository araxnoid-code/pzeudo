...
use crate::{
    Array, ArrayTrait,
    PzeudoNumErr::{self, MulErr},
    shape_to_stride,
};

pub fn mul<Lhs, Rhs, F>(lhs: &Lhs, rhs: &Rhs) -> Result<Array<F>, PzeudoNumErr>
where
    F: Copy + Mul<Output = F>,
    Lhs: ArrayTrait<F>,
    Rhs: ArrayTrait<F>,
{
    ...
    if lhs_metadata.shape != rhs_metadata.shape {
        return Err(MulErr(format!(
            "AddErr. add\ncannot multiply arrays of the form {:?} and {:?} because they are different in shape",
